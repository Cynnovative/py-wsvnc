...
class MockTCPServer(MockTCPBaseServer):
    ...
    def cut_text_message(self, connection: socket):
        text_length = 100000
        header = pack("!BxxxI", 3, text_length)
        self.random_text = randbytes(text_length)
        text = pack(f">{text_length}s", random_text)
        msg = header + text
        connection.send(msg)
    ...
    def handle_client(self, connection: socket, address):
        ...
        try:
            ...
            while True:
                self.cut_text_message(connection)
                connection.recv(2)
                break
        finally:
            connection.close()
