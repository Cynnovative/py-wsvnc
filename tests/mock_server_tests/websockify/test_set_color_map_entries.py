...
class MockTCPServer(MockTCPBaseServer):
    ...
    def color_map_entries_message(self, connection: socket):
        header = pack("!BxHH", 1, 0, 65535)
        colors: bytes = b''
        ...
        for r in range(num_rects):
            ...
            for x in range(100):
                for y in range(100):
                    ...
                    pixel_data += pack(">I", random.randint(0, 65534))
    ...
    def handle_client(self, connection: socket, address):
        ...
        try:
            ...
            while True:
                self.color_map_entries_message(connection)
                #sleep(1)
...
def test():
    ...
    #img.save('test-websockify-color-map-entries.png')
